...
const productFields = "code,product_name,brands,quantity,serving_size,categories_tags,labels_tags,countries_tags,stores_tags,purchase_places_tags,nutriscore_grade,nova_group,ecoscore_grade,ingredients_text,ingredients_tags,ingredients_analysis_tags,allergens_tags,traces_tags,additives_tags,nutriments,data_quality_tags,image_url,last_modified_t"
...
func newSearchCmd(flags *rootFlags) *cobra.Command {
	var category, brand, country, label, nutritionGrade, store, purchasePlace string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			setSearchFilter(query, "stores_tags", store)
			setSearchFilter(query, "purchase_places_tags", purchasePlace)
			if len(query) <= 3 {
			...
			out := map[string]any{
				...
				"query": searchQuerySummary(args, category, brand, country, label, nutritionGrade, store, purchasePlace),
	...
	cmd.Flags().StringVar(&store, "store", "", "Store filter, e.g. publix, sprouts, costco, or aldi")
	cmd.Flags().StringVar(&purchasePlace, "purchase-place", "", "Purchase-place filter from Open Food Facts tags")
...
func newNutritionCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil { return err }
			product := summarizeProduct(response.Product, off.baseURL)
			return writeJSON(cmd, flags, map[string]any{"source":"Open Food Facts Product API v3","barcode":product.Barcode,"nutrition":nutritionSummary(product),"freshness":freshnessFacts(),"caveats":dataCaveats()})
		},
	}
}
...
func newCompareCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return writeJSON(cmd, flags, map[string]any{"source":"Open Food Facts Product API v3","products":products,"errors":errors,"caveats":dataCaveats()})
		},
	}
}
...
func newCategoryCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return writeJSON(cmd, flags, map[string]any{"source":"Open Food Facts Search API v2","category":args[0],"results":results,"count":response.Count,"caveats":searchCaveats()})
		},
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return writeJSON(cmd, flags, map[string]any{
				...
				"api_docs":[]string{"https://openfoodfacts.github.io/openfoodfacts-server/api/","https://openfoodfacts.github.io/openfoodfacts-server/api/ref-cheatsheet/"},
				"base_url":currentConfig().BaseURL,
				"auth":"none for read operations",
				"read_only":true,
				"coverage":[]string{"product lookup","structured search","store filtering","purchase-place filtering","nutrition","ingredients/allergens","small product comparison","category samples"},
				"rate_limits":rateLimitFacts(),
				"freshness":freshnessFacts(),
				"caveats":append(dataCaveats(),"Store/purchase-place tags do not represent live local inventory.","Use CSV/JSONL exports instead of live API calls for bulk datasets."),
				"configured":configuredEnv(),
			})
		},
	}
...
func summarizeProduct(product productRecord, baseURL string) productSummary {
	return productSummary{
		...
		Stores: trimTags(product.StoresTags),
		PurchasePlaces: trimTags(product.PurchasePlacesTags),
		...
		SourceURL: strings.TrimRight(baseURL, "/") + "/product/" + product.Code,
	}
}
...
func selectedNutriments(nutriments map[string]any) map[string]any {
	if len(nutriments) == 0 { return nil }
	...
	for _, key := range keys {
		if value, ok := nutriments[key]; ok { out[key] = value }
	}
	if len(out) == 0 { return nutriments }
	return out
}

func setSearchFilter(query url.Values, key, value string) {
	if strings.TrimSpace(value) != "" { query.Set(key, strings.TrimSpace(value)) }
}

func searchQuerySummary(args []string, category, brand, country, label, nutritionGrade, store, purchasePlace string) map[string]string {
	summary := map[string]string{}
	if len(args) == 1 { summary["terms"] = strings.TrimSpace(args[0]) }
	for key, value := range map[string]string{
		...
		if strings.TrimSpace(value) != "" { summary[key] = strings.TrimSpace(value) }
	}
	return summary
}
...
func trimTags(tags []string) []string {
	...
	for _, tag := range tags {
		...
		if tag != "" { out = append(out, tag) }
	}
	return out
}

func stripLocalePrefix(tag string) string {
	if len(tag) >= 3 && tag[2] == ':' && isASCIIAlpha(tag[0]) && isASCIIAlpha(tag[1]) { return tag[3:] }
	return tag
}
...
func freshnessFacts() []string {
	return []string{
		...
		"The API does not guarantee every product has complete nutrition, ingredient, allergen, store, purchase-place, or image fields.",
	}
}
...
func dataCaveats() []string {
	return []string{
		...
		"Store and purchase-place tags indicate community-recorded associations, not live inventory.",
		"This CLI reports source fields and does not provide medical or dietary advice.",
	}
}
